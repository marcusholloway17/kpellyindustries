...
use App\Services\SequelizemetumService;
use App\Http\ViewModels\SequelizemetumViewModel;
...
final class SequelizemetaController
{
	...
	public function __construct(Validator $validator, ResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new SequelizemetumService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /sequelizemeta[/{$id}]
	 * 
	 * @param SequelizemetumViewModel $viewModel
	 *
	 * @return mixed
	 */
	public function index(SequelizemetumViewModel $viewModel)
	{
		# code...
		...
		$result = $this->service->handle(
			...
			useMapQueryResult(function ($value)  use ($viewModel) {
				return $value ? (new SequelizemetumDto($value))->mergeHidden($viewModel->get('_hidden') ?? []) : $value;
			})
		);
	 ...
	 * @return mixed
	 */
	public function show(SequelizemetumViewModel $viewModel, $id)
	{
		# code...
		...
		$result = $this->service->handle(
			...
			function ($value) use ($viewModel) {
				return null !== $value ? (new SequelizemetumDto($value))->mergeHidden($viewModel->get('_hidden') ?? []) : $value;
			}
		);
		return $this->response->ok($result);
	}
	 ...
	 * @Route /POST /sequelizemeta
	 * 
	 * @param SequelizemetumViewModel $viewModel
	 *
	 * @return mixed
	 */
	public function store(SequelizemetumViewModel $viewModel)
	{
		# code...
		...
		$result = $this->validator->validate($viewModel, function () use ($viewModel) {
			...
			return $this->service->handle(CreateQueryAction($viewModel, [
				...
				return null !== $value ? new SequelizemetumDto($value) : $value;
			});
		});
	 ...
	 * @Route /PUT /sequelizemeta/{id}
	 * @Route /PATCH /sequelizemeta/{id}
	 * 
	 * @param SequelizemetumViewModel $viewModel
	 * @param mixed $id
	 *
	 * @return mixed
	 */
	public function update(SequelizemetumViewModel $viewModel, $id)
	{
		# code...
	 ...
	 * @return mixed
	 */
	public function destroy(SequelizemetumViewModel $viewModel, $id)
	{
		# code...
		// TODO: Provide Policy handlers if required
