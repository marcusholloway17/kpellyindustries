...
use App\Http\ViewModels\MessageViewModel;
...
final class MessagesController
{
	 ...
	 * 
	 * @param MessageViewModel $viewModel
	 *
	 * @return mixed
	 */
	public function index(MessageViewModel $viewModel)
	{
		# code...
		...
		$result = $this->service->handle(
			...
			useMapQueryResult(function ($value)  use ($viewModel) {
				return $value ? (new MessageDto($value))->mergeHidden($viewModel->get('_hidden') ?? []) : $value;
			})
		);
	 ...
	 * @return mixed
	 */
	public function show(MessageViewModel $viewModel, $id)
	{
		# code...
		...
		$result = $this->service->handle(
			...
			function ($value) use ($viewModel) {
				return null !== $value ? (new MessageDto($value))->mergeHidden($viewModel->get('_hidden') ?? []) : $value;
			}
		);
		return $this->response->ok($result);
	}
	 ...
	 * @return mixed
	 */
	public function store(MessageViewModel $viewModel)
	{
		# code...
		...
		$result = $this->validator->validate($viewModel, function () use ($viewModel) {
			...
			return $this->service->handle(CreateQueryAction($viewModel, [
				...
				return null !== $value ? new MessageDto($value) : $value;
			});
		});
	 ...
	 * @return mixed
	 */
	public function update(MessageViewModel $viewModel, $id)
	{
		# code...
	 ...
	 * @return mixed
	 */
	public function destroy(MessageViewModel $viewModel, $id)
	{
		# code...
		// TODO: Provide Policy handlers if required
